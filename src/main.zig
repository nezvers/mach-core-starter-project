...
pub fn init(app: *App) !void {
    ...
    // const sprites_file = try std.fs.cwd().openFile("../../src/sprites.json", .{ .mode = .read_only });
    ...
    // defer root.deinit();
    const root = try std.json.parseFromSlice(JSONData, allocator, assets.sprites_meta, .{});
    defer root.deinit();
    ...
    const shader_module = core.device.createShaderModuleWGSL("sprite-shader.wgsl", assets.sprites_shader);
